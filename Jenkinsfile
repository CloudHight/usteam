        ...
        stage('Trivy report slack notification') {
            steps {
                slackSend(
                    ...
                    message: 'Image Security Scan report generated by Olalere and waiting for review before approval',
                    teamDomain: 'Cloudhight',
            ...
            steps {
                ...
                script {
                    ...
                    if (response == "200") {
                        slackSend(
                            color: 'good',
                            message: "Deployment to stage environment completed successfully. App is up with status code ${response}.",
                            tokenCredentialId: 'slack-cred'
                        )
                    } else {
                        slackSend(
                            color: 'danger',
                            message: "Deployment to stage environment completed, but app appears down. Status code: ${response}.",
                            tokenCredentialId: 'slack-cred'
                        )
                    }
                }
        ...
        stage('check prod website availability') {
            steps {
                script {
                    slackSend(
                        channel: '24th-february-auto-discovery-project',
                        message: 'Starting deployment to prod environment...',
                        tokenCredentialId: 'slack-cred',
                        color: '#FFFF00'
                    )
                }
                ...
                script {
                    ...
                    if (response == "200") {
                        slackSend(
                            color: 'good',
                            message: "Deployment to prod completed successfully. App is up with status code ${response}.",
                            tokenCredentialId: 'slack-cred'
                        )
                    } else {
                        slackSend(
                            color: 'danger',
                            message: "Deployment to prod completed, but app appears down. Status code: ${response}.",
                            tokenCredentialId: 'slack-cred'
                        )
                    }
                }
    ...
    post {
        failure {
            script {
                slackSend(
                    channel: '24th-february-auto-discovery-project',
                    message: "Pipeline failed during job execution",
                    tokenCredentialId: 'slack-cred',
                    color: '#FF0000'
                )
            }
        ...
        success {
            script {
                slackSend(
                    channel: '24th-february-auto-discovery-project',
                    message: "Pipeline executed successfully on Jenkins Node Slave",
                    tokenCredentialId: 'slack-cred',
                    color: '#36A64F'
                )
            }
...
//         stage('Trivy report slack notification') {
//             steps {
//                 slackSend channel: '24th-february-auto-discovery-project', message: 'Image Security Scan report generated by Olalere and waiting for review before approval ', teamDomain: 'Cloudhight', tokenCredentialId: 'slack-cred'
//             }
//         }
//         stage('Security cleared Approval (trivy)') {
//             steps {
