pipeline {
    ...
    stages {
        ...
        stage('check stage website availability') {
            steps {
                 sh "sleep 90"
                 sh "curl -s -o /dev/null -w \"%{http_code}\" http://stage.chijiokedevops.space"
                script {
                    def response = sh(script: "curl -s -o /dev/null -w \"%{http_code}\" http://stage.chijiokedevops.space", returnStdout: true).trim()
                    if (response == "200") {
        ...
        stage('check prod website availability') {
            steps {
                 sh "sleep 90"
                 sh "curl -s -o /dev/null -w \"%{http_code}\" http://www.chijiokedevops.space"
                script {
                    def response = sh(script: "curl -s -o /dev/null -w \"%{http_code}\" http://www.chijiokedevops.space", returnStdout: true).trim()
                    if (response == "200") {
